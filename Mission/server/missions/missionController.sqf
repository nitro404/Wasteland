...
while { true } do {
	if(_missionFunctionType == 0) then {
		_vehicleList = (_missionTypeInfo select 5) select (floor (random count (_missionTypeInfo select 5)));
		...
		_activeMission = [_missionTypeInfo select 1, _vehicleClass, _missionTypeInfo select 3, _missionTypeInfo select 4, missionNumber] spawn vehicleMission;

		diag_log format["WASTELAND SERVER - Execute New %1 (%2)", _missionTypeInfo select 1, getText (configFile >> "cfgVehicles" >> _vehicleClass >> "displayName")];

		[nil, nil, rHINT, parseText format ["<t align='center' color='%4' shadow='2' size='1.75'>%1</t><br/><t align='center' color='%4'>------------------------------</t><br/><t align='center'><img size='5' image='%2'/></t><br/><t color='%5' size='1.0'>Starting in %3 Minutes</t>", _missionTypeInfo select 1, _missionRewardPicture, (_missionTypeInfo select 3) / 60, "#52bf90", "#FFFFFF"]] call RE;
	};
	...
	if(_missionFunctionType == 1) then {
		...
		[nil, nil, rHINT, parseText format ["<t align='center' color='%3' shadow='2' size='1.75'>%1</t><br/><t align='center' color='%3'>------------------------------</t><br/><t color='%4' size='1.0'>Starting in %2 Minutes</t>", _missionTypeInfo select 1, (_missionTypeInfo select 3) / 60, "#52bf90", "#FFFFFF"]] call RE;
	};
