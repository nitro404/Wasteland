...
for "_iteration" from 1 to _lockDuration do  {
	...
	if(player distance (nearestobjects [player, ["EvMoney"], _maxMoneyDistance] select 0) > _maxMoneyDistance) exitWith {
		mutexScriptInProgress = false;
	...
	if(_iteration >= _totalDuration) exitWith {
		sleep 1;
		_currMoneyTemp = (nearestObjects [getpos player, ["EvMoney"], _maxMoneyDistance]);
		...
		if(count _currMoneyTemp == 0) then {
			...
			_money = ((nearestobjects [getpos player, ["EvMoney"], _maxMoneyDistance] select 0) getVariable "money");
